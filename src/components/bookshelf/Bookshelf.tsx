...
import { PetPopup, ShelfPet } from "./CatFigurine";
...
export function Bookshelf() {
  ...
  const showShelfPet = true;
  const showShelfPlant = true;
  ...
  return (
    ...
    >
      ...
      <main className="relative z-10 mx-auto mt-10 w-full max-w-5xl px-2 md:mt-16 md:px-12">
        ...
        {(showShelfPet || showShelfPlant) && (
          <div className="mt-8 flex items-end justify-center gap-10 px-6 sm:gap-20">
            ...
            {showShelfPet && (settings.petsConfig["shelf"]?.pet || !settings.petDismissed) && (
      ...
      {openBook && <BookOpen book={openBook} onClose={() => setOpenId(null)} basicMode={viewMode === "simple"} />}
    </div>
  );
}
